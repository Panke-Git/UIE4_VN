"""Train v17 with fixed train/validation manifests; held-out test images are never loaded."""
...
EXPECTED_VERSION = "v17"
DEFAULT_CONFIG = "configs/config_v17.yaml"
...
def _load_config(args: argparse.Namespace) -> tuple[dict, Path, Path | None]:
    if args.resume:
        ...
        config = apply_overrides(config, data_root=args.data_root)
    ...
    config = apply_overrides(load_yaml(source), args.seed, args.data_root, args.name)
...
def main(argv: list[str] | None = None) -> None:
    ...
    try:
        ...
        if config["scheduler"]["name"].lower() != "none":
            raise ValueError("Only scheduler.name=none is implemented in phase one")
        scheduler = None
